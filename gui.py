...
class workThread(QThread):  # 视频合成的多线程
    ...
    def __init__(self, watermark, video, filname, sen, samolelist, peroid, filetype, kbps, maxkbps, videotype,
                 outputtype, processtype, watermarkquality):
        super().__init__()
        print("run")
...
class MainWindows(QMainWindow, Ui_MainWindow):
    ...
    def browservideofunc(self):
        self.videopath = QFileDialog.getOpenFileName(self, "选择输入视频文件", "",
                                                     "mp4 文件(*.mp4);;mkv 文件(*.mkv);;avi 文件(*.avi);;flv 文件(*.flv)")
    ...
    def start(self):
        self.inputp()
        self.work = workThread(self.wmpath1, self.videopath1, self.videoname, 0, self.sampletimes, self.peroids,
                               self.mtype, self.deskpbs, self.mkpbs, self.videotype, self.outtype, self.wmtype,
                               int(self.wmquality))
        self.work.start()
