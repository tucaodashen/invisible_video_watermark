...
def extractall(video, filepartern=".png"):  # 提取所有视频帧
    ...
    cap.release()
    result = r"ffmpeg\bin\ffmpeg.exe -i " + str(video) + " -vf " + '"fps=' + str(
        fps) + '"' + " origin/%0d" + filepartern
...
def extractaudio(video):  # 提取音频
    try:
        ...
        st = os.system(
            r"ffmpeg\bin\ffmpeg.exe -i " + video + " -vn -c:a copy origin/" + str(os.path.basename(video)) + ".aac")
    except:
        st = "err"
    return st
...
def output(origin, video, fps, sen, kbps=10000, maxkbps=15000, mtype=".jpg", vtype=".mp4", audstate=0):  # 视频合成
    """
    进行ffmpeg合成
    :param origin:原视频路径
    :param video: 导出视频名
