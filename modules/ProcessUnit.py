...
from modules.AudioProcessor import extract_audio_to_flac, get_audio_tracks_info
...
class ProcessUnit(QObject):
    ...
    def audio_process(self, input_video, path):
        ...
        self.logger.info(f"Video {input_video} has {track_count} audio tracks",
                         tags=f"ProcessUnit:ProcessUnit:extract_audio_to_flac:{os.path.basename(input_video)}")
    ...
    def set_args(self, **kwargs):
        for key, value in kwargs.items():
            if key == "version" and value != const.__version__:
                logger.error(f"Version {value} not match {const.__version__}",
                             tags=f"ProcessUnit:ProcessUnit:set_args:{os.path.basename(self.file)}")
    ...
    def search_dump_file(self):
        ...
        logger.debug(f"Found dump file: {self.dump_file}",
                     tags=f"ProcessUnit:ProcessUnit:search_dump_file:{os.path.basename(self.file)}")
    ...
    def setup_ipc(self):
        ...
        self.ipc_port = port
        self.logger.info(f"Set up IPC on port: {self.ipc_port}",
                         tags=f"ProcessUnit:ProcessUnit:setup_ipc:{os.path.basename(self.file)}")
        ...
        try:
            ...
            if data["process_order"] is None and data['process_message'] is not None:
                self.logger.debug(f"Received message: {data['process_message']}",
                                  tags=f"ProcessUnit:ProcessUnit:ipc_callback:{os.path.basename(self.file)}")
    ...
    def set_up_progress_dict(self):
        for i in range(len(self.slice_list) + 1):
            self.progress_dict.update({i: 0})

    def sample(self):
        self._sample_list = VideoProcessor.video_sampler(self.file, self.sample_times, self.sample_extend,
                                                         self.sample_type, self.manual_sample_sheet)

    def generate_queue(self):
        count = self.frame_count
        self._sliced_list = VideoProcessor.spitter(count, self.slice_length)
        self.logger.debug(f"Slice list length: {len(self._sliced_list)}",
                          tags=f"ProcessUnit:ProcessUnit:generate_queue:{os.path.basename(self.file)}")
        ...
        self.logger.debug(f"Length of slice list: {len(self.slice_list)}",
                          tags=f"ProcessUnit:ProcessUnit:generate_queue:{os.path.basename(self.file)}")

    def prepare_for_merge(self):
        self.logger.debug(f"Output path: {self.output_path}",
                          tags=f"ProcessUnit:ProcessUnit:prepare_for_merge:{os.path.basename(self.file)}")
        ...
        self.logger.debug("Create directory for audio track",
                          tags=f"ProcessUnit:ProcessUnit:prepare_for_merge:{os.path.basename(self.file)}")
        ...
        self.logger.success(f"Prepare for merge success, output path: {self.output_path}",
                            tags=f"ProcessUnit:ProcessUnit:prepare_for_merge:{os.path.basename(self.file)}")
    ...
    def merge(self):
        ...
        for i in range(1, ti + 1):
            self.saved_file_path = FileSystem.open_file(f"{self.base_file_name}-merge", f"./merge/{i}.{self.output_format}",
                                                        const.File_Return_Type.PATH)
            lists.append(FileSystem.open_file(f"{self.base_file_name}-merge", f"./merge/{i}.{self.output_format}",
                                              const.File_Return_Type.PATH))
        ...
        merge_video_sequnece(lists, os.path.join(self._temporary_path, f"{self.output_name}.{self.output_format}"),
                             logger_inst=self.logger, audio_file=self.audio_file_list, output_format=self.output_format)
    ...
    def report_error(self, error_list):
        ...
        self.logger.critical(f"Report error: {error_list}",
                             tags=f"ProcessUnit:ProcessUnit:report_error:{os.path.basename(self.file)}")

    @timer_decorator
    def run(self):
        self.logger.info(f"Start process unit: {self.identify} with configuration {self.preference_args} and description {self.description}",
                         tags=f"ProcessUnit:ProcessUnit:run:{os.path.basename(self.file)}")
        self.setup_ipc()
        self.set_stage(0)
        self.sample()
        self.set_stage(1)
        ...
        if not self.error_occured and self.result_list[0] != 'Terminated':
            ...
            self.running = False
            self.status = 1
            self.logger.success(f"Process unit: {self.identify} completed",
                                tags=f"ProcessUnit:ProcessUnit:run:{os.path.basename(self.file)}")
            return True
        else:
            ...
            self.running = False
            self.status = 0
            self.logger.critical(f"Process unit: {self.identify} error: {self.completed}",
                                 tags=f"ProcessUnit:ProcessUnit:run:{os.path.basename(self.file)}")
            try:
                self.remove_workspace()
            except Exception as e:
                self.logger.critical(f"Remove workspace error: {e}",
                                     tags=f"ProcessUnit:ProcessUnit:run:{os.path.basename(self.file)}")
                pass
            return False
    ...
    def stop(self):
        ...
        self.logger.info(f"Stop process: {self.scheduler.manager_pid}",
                         tags=f"ProcessUnit:ProcessUnit:stop:{os.path.basename(self.file)}")
        self.stopped = True
    ...
    def after_processing(self):
        ...
        self.logger.debug(f"Recover data: {recover_data}",
                          tags=f"ProcessUnit:ProcessUnit:after_processing:{os.path.basename(self.file)}")
        if os.path.exists(json_path):
            os.remove(json_path)
        with open(json_path, "wb") as f:
            f.write(data)
        self.logger.debug(f"Recover data saved to: {json_path}",
                          tags=f"ProcessUnit:ProcessUnit:after_processing:{os.path.basename(self.file)}")
    ...
    def set_stage(self, stage):
        ...
        self.logger.info(f"Set process unit stage: {self.process_unit_stage}",
                         tags=f"ProcessUnit:ProcessUnit:set_stage:{os.path.basename(self.file)}")
    ...
    def process_attachment(self):
        ...
        self.logger.debug(f"Attachment data: {list_data}",
                          tags=f"ProcessUnit:ProcessUnit:process_attachment:{os.path.basename(self.file)}")
        return list_data
    ...
    def remove_workspace(self):
        ...
        shutil.rmtree(root)
        self.logger.debug(f"Remove workspace: {root}",
                          tags=f"ProcessUnit:ProcessUnit:remove_workspace:{os.path.basename(self.file)}")
