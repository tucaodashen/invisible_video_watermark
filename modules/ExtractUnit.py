...
class ExtracUnit(QObject):
    ...
    def search_dump_file(self):
        ...
        logger.debug(f"Found dump file: {self.dump_file}",tags=f"ExtractUnit:ExtractUnit:search_dump_file:{os.path.basename(self.video)}")
    ...
    def generate_slice(self):
        ...
        for ranges in a:
            ...
            logger.debug(f"append slice {id} {ranges}",tags=f"ExtractUnit:ExtractUnit:generate_slice:{os.path.basename(self.video)}")


    def report_error(self,*args):
        print(args)
        self.search_dump_file()
        self.error_occured.emit([args,self.dump_file])
        logger.critical(f"error occured {args}",tags=f"ExtractUnit:ExtractUnit:report_error:{os.path.basename(self.video)}")
    ...
    def run(self):
        ...
        logger.debug(f"extract slice running over",tags=f"ExtractUnit:ExtractUnit:run:{os.path.basename(self.video)}")
        return _result
    ...
    def run_debug(self):
        ...
        for i in self._slice_list:
            ...
            logger.debug(f"extract slice {i.identify} {i.frame_range} result {i._result}",tags=f"ExtractUnit:ExtractUnit:run_debug:{os.path.basename(self.video)}")
    ...
    def post_process(self,result):
        if self.plk_data['watermark_method'] == WatermarkAlgorithm.IMAGE_GUOFEI or self.plk_data['watermark_method'] == WatermarkAlgorithm.IMAGE_FIREKEEPER:
            ...
            for imgset in sorted:
                if len(imgset) >= 1:
                    ret_result += imgset
            logger.debug(f"post processed length {len(ret_result)}",tags=f"ExtractUnit:ExtractUnit:post_process:{os.path.basename(self.video)}")
        else:
            text_result = []
            for texts in result:
                if texts:
            ...
            logger.debug(f"post process {ret_result}",tags=f"ExtractUnit:ExtractUnit:post_process:{os.path.basename(self.video)}")
    ...
    def sort_result(self,result,thresholds=0.9):
        ...
        logger.debug(f"save {len(ahh_result)} images to {path}",tags=f"ExtractUnit:ExtractUnit:sort_result:{os.path.basename(self.video)}")
...
class ExtractSlice:
    ...
    def start(self):
        try:
            ...
            if exc_traceback:
                ...
                # 只转储异常发生的帧
                logger.error(f"extract slice {self.identify} {self.frame_range} error {e}",tags=f"ExtractUnit:ExtractSlice:start:{os.path.basename(self.video)}:{self.identify['order']}")
                coredumpy.dump(frame=exception_frame,
                               description="Exception trigger frame only"
                               , path=path, depth=4)

                # 打印确认位置
                logger.error(f"exception frame {exception_frame.f_code.co_filename}:{exception_frame.f_lineno}",tags=f"ExtractUnit:ExtractSlice:start:{os.path.basename(self.video)}:{self.identify['order']}")
            else:
                logger.error(f"extract slice {self.identify} {self.frame_range} error {e}",tags=f"ExtractUnit:ExtractSlice:start:{os.path.basename(self.video)}:{self.identify['order']}")
    ...
    def decode(self):
        logger.debug(f"decode {self.identify} {self.frame_range}",tags=f"ExtractUnit:ExtractSlice:decode:{os.path.basename(self.video)}:{self.identify['order']}")
